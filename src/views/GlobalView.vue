...
const globalResult = inject('globalResult');
const _globalResult = inject('_globalResult');
 ...
 * Filter
 */
const showFillRateFilter = ref(false);
const fillRateCriteria = ref([0, 100]);
...
const exportData2 = (type, filename) => {
  csvXlsx.exportData2(type, filename, query.value, serverItemsLength.value,
    ['Stations', 'Category', 'Nights', 'Beds', 'Lots', 'Fill rate', 'Occupancy rate', 'Period']);
  downloaded.value = true;
}
 ...
 * Searching
 */
const searchField = ["stations", "category", "period"];
...
const nightPerWeek = async(item)=>{
  console.log(item)
   console.log(`/frequentations/weeks?station=${item.station_code}&category=${item.category}&period=${item.period}`)
   const response = await new Promise((resolve, reject) => {
